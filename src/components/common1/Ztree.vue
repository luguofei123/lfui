...
export default {
      ...
      handler: function(nodes){
        ...
        this.$nextTick(() => {
          ...
          if(this.selectId!= ''){
            ...
            if(this.selectId.indexOf(' ')>-1){
              this.initAgencyAcctScroll('codeName');
            }else{
              this.initAgencyAcctScroll('code');
            }
          }
          if(this.checkedCodes.length > 0){
            let nodes = this.ztreeObj.getNodesByFilter(node => {
              return this.checkedCodes.some(code => {
                       return code === node[this.keyProp]
                     });
            })
            nodes.forEach(node => {
  ...
  methods: {
    initAgencyAcctScroll(codeItem) {
      ...
      allNodes.forEach(node => {
        allNodesArr.push(node[codeItem])
        //将所有节点变成不高亮
        node.highlight = false;
        this.ztreeObj.updateNode(node);
      })
      ...
      this.ztreeObj.refresh();
      //搜索的对应codeItem的节点
      let searchNodes = this.ztreeObj.getNodesByParam(codeItem, this.selectId, null);
      if(searchNodes.length > 0){
        searchNodes[0].highlight = true
        //该节点高亮
        this.ztreeObj.updateNode(searchNodes[0])
        let index = allNodesArr.indexOf(searchNodes[0][codeItem].toString());
