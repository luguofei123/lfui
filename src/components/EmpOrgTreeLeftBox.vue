 ...
 * @LastEditTime: 2020-12-16 15:04:45
    ...
    <div class="searchWrap">
      <a-input-search class="orgTreeSearch" allowClear ref="filterText" v-model="searchText" @search="onSearch(searchText)" placeholder="请输入关键词">
        <a-button class="flex-c-c" slot="enterButton">
          <i class="icon-search" style="padding-top: 5px;" ref="searchIcon"></i>
        </a-button>
      </a-input-search>
    </div>
...
export default {
  ...
  methods: {
    ...
    onClick(...arg) {
      ...
      // 不是叶子节点 人员传空，部门传code ''
      if (arg[2].isLeaf == '0') {
        this.$emit('select', arg[2], 'orgCode')
      // 叶子节点，说明是人员，传人员code和父id
      } else {
        this.$emit('select', arg[2], 'empCode')
      }
      // if(arg[2].pId === '0'){
